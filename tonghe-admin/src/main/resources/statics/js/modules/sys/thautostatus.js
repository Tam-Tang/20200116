$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'sys/thautostatus/list',
        datatype: "json",
        colModel: [			
	...
	methods: {
		...
		saveOrUpdate: function (event) {
		    $('#btnSaveOrUpdate').button('loading').delay(1000).queue(function() {
                var url = vm.thAutoStatus.id == null ? "sys/thautostatus/save" : "sys/thautostatus/update";
                $.ajax({
                    type: "POST",
                    url: baseURL + url,
                    contentType: "application/json",
                    data: JSON.stringify(vm.thAutoStatus),
                    success: function(r){
                        if(r.code === 0){
		...
		del: function (event) {
            ...
            layer.confirm('确定要删除选中的记录？', {
               ...
               if(!lock) {
		            ...
		            $.ajax({
                        type: "POST",
                        url: baseURL + "sys/thautostatus/delete",
                        contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "sys/thautostatus/info/"+id, function(r){
                vm.thAutoStatus = r.thAutoStatus;
            });
		},
